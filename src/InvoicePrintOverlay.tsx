...
<div class="footer"><span>RESQ Mobile Clinic</span><span>Invoice ${esc(i.id)} · <span class="accent">${date(i.date)}</span></span></div></div></body></html>`);
  w.document.close();
  // The previous implementation printed after a fixed 350ms delay, which can race
  // the logo request and produce a broken-image/question-mark placeholder. Wait for
  // the actual image load/decode before invoking the print dialog.
  const img=w.document.querySelector<HTMLImageElement>('.logo');
  const doPrint=()=>{w.focus();w.print()};
  if(!img){setTimeout(doPrint,100);return;}
  const fallback=setTimeout(doPrint,3000);
  img.onload=async()=>{clearTimeout(fallback);try{if(img.decode)await img.decode()}catch{}doPrint()};
  img.onerror=()=>{clearTimeout(fallback);w.close();window.alert('Unable to load the RESQ invoice logo. Please refresh the application and try again.')};
