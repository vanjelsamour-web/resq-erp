...
function printInvoice(i:Invoice){
  ...
  // Use the exact supplied RESQ logo from /public. Direct WebP avoids the broken
  // embedded-WebP-in-SVG placeholder shown by some print renderers.
  const logoUrl=`${window.location.origin}/resq-logo.webp?v=2`;
...
<div class="footer"><span>RESQ Mobile Clinic</span><span>Invoice ${esc(i.id)} · <span class="accent">${date(i.date)}</span></span></div></div></body></html>`);
  w.document.close();
  // Wait for the real image load instead of printing after a fixed delay.
