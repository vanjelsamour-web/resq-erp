...
export default function Cases({cases,patients,onAdd,onStatus}:Props){
 ...
 return <><style>{`.case-patient-picker{position:relative}.case-picker-input{max-width:none;width:100%;padding-right:7px}.case-picker-input input{padding:10px 0}.case-picker-clear{border:0;background:transparent;color:#8191a2;padding:5px;display:grid;place-items:center}.case-patient-results{position:absolute;z-index:120;top:calc(100% + 5px);left:0;right:0;background:#fff;border:1px solid #d9e1e8;border-radius:8px;box-shadow:0 12px 30px #071a2b1c;max-height:260px;overflow:auto}.case-patient-results button{width:100%;border:0;border-bottom:1px solid #edf0f3;background:#fff;text-align:left;padding:10px 12px;display:grid;grid-template-columns:auto 1fr;column-gap:8px;row-gap:3px;color:#172033}.case-patient-results button:last-child{border-bottom:0}.case-patient-results button:hover{background:#f4f7fa}.case-patient-results strong{font-size:11px;color:#2477d4}.case-patient-results span{font-size:12px;font-weight:600}.case-patient-results small{grid-column:1/-1;color:#8191a2;font-size:10px}.case-picker-empty{padding:14px;color:#8191a2;font-size:12px}`}</style><div className="page-heading"><div><p className="eyebrow">CASE MANAGEMENT</p><h1>Cases</h1><p className="muted">Track cases, decisions, amounts and supporting documentation.</p></div><button className="primary" onClick={()=>{setError('');setPatientSearch('');setForm({patientId:'',type:'Consultation',amount:'',notes:''});setShow(true)}}><Plus size={17}/> New case</button></div>
